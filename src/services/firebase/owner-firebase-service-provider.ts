...
import { OwnerModel, ResidentModel } from '../../models';

@Injectable()
export class OwnerFirebaseServiceProvider {
    // https://www.youtube.com/watch?v=-GjF9pSeFTs

    private tableName: string = 'owner';

    constructor(private db: AngularFirestore) {}

    public insertRecord(model: OwnerModel, callbackMethod) {
        this.db
    ...
    public getAll(estateKey: string, callbackMethod) {
        ...
        let snapshot = collectionRef.snapshotChanges().map((changes) => {
            return changes.map((snap) => {
                return snap.payload.doc.data() as OwnerModel;
            });
        });
    ...
    public updateRecord(model: OwnerModel, callbackMethod) {
